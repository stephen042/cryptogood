<div
    ...
    <div
        ...
        <h2 style="font-size:20px; font-weight:600; color:white;">Buy / Receive History (Deposit)</h2>
    </div>
        ...
        <table style="width:100%; font-size:14px; text-align:left; color:white; border-collapse:collapse;">
            <thead>
                <tr style="font-weight:600; background:#1f2937;">
                    ...
                    <th style="padding:12px 24px;">Proof IMG</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #374151; transition:background 0.3s;">
                    ...
                    <td style="padding:12px 24px; color:white;">{{ $deposit->created_at->format('d M, Y') }}</td>
                    <td style="padding:12px 24px; font-weight:500; color:white;">${{ number_format($deposit->amount, 2)
                        }}</td>
                    <td style="padding:12px 24px; color:white;">
                        @if ($deposit->proof)
                        ...
                        @else
                        <span style="color:#ef4444;">No Image</span>
                        @endif
                    </td>
                    <td style="padding:12px 24px; color:white;">
                        <span style="padding:4px 12px; font-size:12px; font-weight:600; border-radius:9999px;
                            @if($deposit->status == 3) background:#16a34a; color:white;
                            @elseif($deposit->status == 1) background:#eab308; color:white;
                    ...
                    <td style="padding:12px 24px; color:white;">
                        <div style="display:flex; gap:8px;">
                            @if ($deposit->status == 3 || $deposit->status == 2)
                            <!-- Delete Button -->
